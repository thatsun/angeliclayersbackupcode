...
public class C_main_menu_control : MonoBehaviour {
    ...
    public Sprite[] disabledsprites = new Sprite[7];
			...
			for(int i=0; i < misiones_botones.Length; i++ ){
				if (game_control.control.catalogo_misiones [i].unlocked == 1) {
					...
					if (game_control.control.catalogo_misiones [i].cleared == 1) {
                        ...
                        imagenes_botones[i].sprite = disabledsprites[i];
	...
	public void nextscene_mision(int mision) {
		...
		game_control.control.cinematico1  = cinematico1 [mision];
		game_control.control.cinematico2  = cinematico2 [mision];
